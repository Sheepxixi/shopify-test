...
<script>
  async function createOrderFromQuote(quote) {
    try {
      ...
      // 1. 调用后端 API 创建草稿订单
      const response = await fetch('https://shopify-test-brown.vercel.app/api/create-draft-order', {
        method: 'POST',
      ...
      if (response.ok && result.success) {
        // 2. 更新 Metaobject 状态
        await fetch('https://shopify-test-brown.vercel.app/api/quotes?handle=' + quote.handle, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            status: 'Ordered',
